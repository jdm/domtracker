...
function ModPlayer(mod, rate) {
	...
	this.ticksPerFrame = 0; /* calculated by setBpm */
	this.ticksPerOutputSample = Math.round(this.ticksPerSecond / rate);
...
ModPlayer.prototype = {
  ...
  prepareChannel: function(channel, note) {
	...
	channel.noteNumber = ModPeriodToNoteNumber[channel.prevNote.period];
	channel.ticksPerSample = ModPeriodTable[channel.finetune][channel.noteNumber] * 2;
      }
    }          
  },
  ...
  loadRow: function(rowNumber) {
	...
	channel.sampleNum = 0;
      ...
      if (note.period != 0 || note.sample != 0) {
        this.prepareChannel(channel, note);
      }
  ...
  getNextRow: function() {
    ...
    if (this.doBreak) {
      ...
      this.breakPos = (this.breakPos >= this.mod.positionCount) ? this.mod.positionLoopPoint : this.breakPos;
  ...
  getSamples: function(sampleCount, advanceFrame) {
	...
	this.ticksSinceStartOfFrame -= this.ticksPerFrame;
      ...
      samples[i+1] = rightOutputLevel / (128 * 128 * this.mod.channelCount);
      i += 2;
    }
